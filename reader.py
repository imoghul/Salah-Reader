...
def processWordsPdf(words):
    ...
    for i in [i for i in range(len(words)) if words.startswith(':', i)]:
        ...
        while words[counter:counter+8]!="Salaatul":
            counter-=1
        if words[counter+8:counter+11]=="Faj": salaah="Fajr"
        elif words[counter+8:counter+11]=="Thu": salaah="Thuhr"
...
def getTimes():
    for i in glob.glob("iqaamahdoc/*.pdf"): return processWordsPdf(pdf2words("iqaamahdoc/Iqaamah Times.pdf"))

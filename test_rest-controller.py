...
def set_led_state(led_colour, led_new_state):
    ...
    if not led_new_state in state:
        return json.dumps({"result": "error - incorrect state option supplied"})
...
class helpPageResource:
    def on_get(self, req, resp):
        ...
        resp.text = (
            "Hello! To interact with this API please use GET and PUT requests on:\n\n"
            + req.url
            + "<led_colour>\n\n"
            "Available colours:\n"
            "    /green, /red\n"
...
green_resource = LedGreenResource()
red_resource = LedRedResource()

app.add_route("/", help_page)
app.add_route("/green", green_resource)
app.add_route("/red", red_resource)
